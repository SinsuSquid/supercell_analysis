...
long double getDisplacement(timestep, timestep);
...
int main(int argc, char* argv[])
{
	...
	for (int i = 1; i < maxTimestep; i++){
		if (i < 300) continue; // After 3ps equilibration
		if (i % 100 == 0) printf("Calculating MSD for t = %d...\n", i);
		for (int j = 1; j <= maxTimestep - i; j++){
			squared[i] += getDisplacement(trajectory[j+i], trajectory[j]);
			numData[i] += 1;
		}
		squared[i] /= numData[i];
...
long double getDisplacement(timestep start, timestep this){
	long double displacement = 0;
	long double da, db, dc;
	long double dx, dy, dz;
	
	for (int i = 0; i < numParticle; i++){
		da = this.particle[i].x - start.particle[i].x;
		db = this.particle[i].y - start.particle[i].y;
		dc = this.particle[i].z - start.particle[i].z;
	...
	return displacement / numParticle;
}
